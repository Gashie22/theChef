...
window.onload=getCartData(key)
  async function getCartData(key){
    let res = await fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/view/${key}`)
 ...
 function appendCartData(data) {
  ...
  function removeProductFromCart(vegId,key){
    ...
    redirect: 'follow'
      ...
      fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/remove/${vegId}/${key}`, requestOptionsOfDelete)
...
//Increase Quantity By 1 Function
 function increaseVegQuantiy(vegId,key){
  ...
  fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/ince/${vegId}/${key}`, requestOptionsOfIncr)
   .then(response => response.text())
  .then(result => console.log(result))
  .catch(error => console.log('error', error));
    getCartData(key);
  ...
  function decreaseVegQuantity(vegId,key){
    ...
    fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/decr/${vegId}/${key}`, requestOptionsOfDecr)
   .then(response => response.text())
   .then(result => console.log(result))
   .catch(error => console.log('error', error));
   getCartData(key);
...
fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/count/${key}`, countRequestOptions)
  .then(response => response.text())
  .then(count => {
...
fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/getTotalAmount/${key}`, subTotalRequestOptions)
  .then(response => response.text())
  .then(result => {
    console.log(result)
    if(result>0){
 ...
 async function userLogout(event){
   ...
   fetch(`https://theChef vegetables and Meatsdeployment-production.up.railway.app/userlogout/${key}`, logoutRequestOptions)
     .then(response => response.text())
     .then(result => {
       console.log(result);
       user.splice(0,1);
